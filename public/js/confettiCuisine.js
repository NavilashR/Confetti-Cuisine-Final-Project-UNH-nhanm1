$(document).ready(() => {
  ...
  $("#modal-vacation").click(() => {
    $(".modal-body").html("");
    $.get(`/api/vacations`, (results = {}) => {
      let data = results.data;
      if (!data || !data.vacations) return;
      data.vacations.forEach(vacation => {
        $(".modal-body").append(
          `<div>
						...
						<span class="course-cost">$${vacation.cost}</span>
						<button class="${vacation.joined ? "joined-button" : "join-button"} btn btn-info btn-sm" data-id="${
            vacation._id
          }">
							${vacation.joined ? "Joined" : "Join"}
						</button>
						<div class="course-description">
